...
export default function NakesPage() {
  const [statusGizi, setStatusGizi] = useState<any[]>([]);
  const [stuntingPerRT, setStuntingPerRT] = useState<any[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      if (result.success) {
        setStatusGizi(result.data.statusGizi);
        setStuntingPerRT(result.data.stuntingPerRT);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Jumlah Balita" value={stats.totalStunting || 120} satuan="anak terdaftar" barColor="red" progress={75} sparkData={[110,112,115,117,119,120]} />
        ...
        <StatCard label="Posyandu Aktif" value={stats.totalPosyandu || 8} satuan="unit posyandu" barColor="green" progress={80} />
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={stuntingPerRT} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ fontSize: 11, borderRadius: 8 }} formatter={(v: any) => [`${v}%`, 'Stunting']} />
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm font-semibold flex items-center gap-2" style={{ color: COLOR }}>
              <HeartPulse size={16} /> Tren Pemantauan Kesehatan (6 Bulan)
            </CardTitle>
            <Link href="/nakes/ibu-hamil" className="text-xs text-indigo-600 hover:text-indigo-800 font-medium transition-colors">
              Detail →
            </Link>
          </div>
        </CardHeader>
