// Script untuk seed data Layanan SLV melalui API
// Jalankan dengan: node scripts/seed-layanan-slv-api.js

const API_BASE = 'http://localhost:3000/api/module-records';
...
const moduleRecords = [
  ...
  },
  {
    ...
    valueText: "Peserta: 150 | Status: Voting Berlangsung | Keputusan: Belum",
    status: "Aktif",
    createdBy: "lembaga_adat"
  },
  {
    modulePath: "/layanan-slv/musyawarah-digital",
    moduleName: "Musyawarah Digital",
    title: "Penetapan Anggaran Desa 2025",
    category: "Agenda",
    description: "Musyawarah penetapan anggaran dan alokasi dana desa",
    valueText: "Peserta: 120 | Status: Selesai | Keputusan: Disetujui",
    status: "Selesai",
    createdBy: "lembaga_adat"
  },
  {
    ...
    valueText: "Suara Setuju: 89 | Suara Tolak: 12 | Total: 101",
    status: "Aktif",
    createdBy: "lembaga_adat"
  },
  {
    modulePath: "/layanan-slv/partisipasi-musyawarah",
    moduleName: "Partisipasi Musyawarah",
    title: "Penetapan Jadwal Adat",
    category: "Agenda",
    description: "Musyawarah untuk penetapan jadwal kegiatan adat bulan Juli",
    valueText: "Suara Setuju: 54 | Suara Tolak: 8 | Total: 62",
    status: "Aktif",
    createdBy: "lembaga_adat"
  },
...
async function seedData() {
  console.log("🚀 Memulai seed data Layanan SLV melalui API...");
  console.log(`📦 Total ${moduleRecords.length} record akan diinsert\n`);
  ...
  for (const record of moduleRecords) {
    try {
      ...
      if (response.ok) {
        ...
        console.log(`⚠️  Gagal: ${record.moduleName} - ${record.title} - ${error}`);
      }
    } catch (error) {
      errorCount++;
      console.log(`⚠️  Error: ${record.moduleName} - ${record.title} - ${error.message}`);
    }
